...
public interface PostRepository extends JpaRepository<MyPost, Long>{
    ...
    @Query("SELECT new hu.project.groupproject.dtos.postDTOs.out.PostDTOPublic(p.id, p.user.id, p.user.userName, p.org.id, p.org.name, p.content, p.url, p.imagePath, p.likes, p.dislikes, p.vote.id) FROM MyPost p LEFT JOIN p.user LEFT JOIN p.org WHERE p.id=:id")
