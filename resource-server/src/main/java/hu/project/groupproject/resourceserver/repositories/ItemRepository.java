...
public interface ItemRepository extends JpaRepository<MyItemForSale, String> {
    ...
    @Query("SELECT new hu.project.groupproject.resourceserver.dtos.En.ItemDtoPublicPartial(i.id, i.user.id, i.name, i.description, i.condition, i.location, i.phone, i.price) FROM MyItemForSale i LEFT JOIN i.user WHERE i.id=:id")
