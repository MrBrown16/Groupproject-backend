...
public class NewsService {
    ...
    public NewsService(NewsRepository newsRepository, VoteService voteService, UserService userService){
    ...
    public NewsDtoPublic updateNews(String newsId, NewsDtoCreate newsUpdate) throws NotFoundException{
            ...
            if (oldNews.isPresent()) {
                ...
                if (canEditNews(newsId, newsUpdate)) {
                    ...
                    return new NewsDtoPublic(news.getId(), news.getUser().getId(), news.getUser().getUserName(), news.getOrg().getId(), news.getOrg().getName(), news.getTitle(), news.getContent(), news.getType());
                }
    ...
    public NewsDtoPublic createNews(NewsDtoCreate newsCreate){
        MyNews news = new MyNews();
        if (canEditNews(null, newsCreate)) {
    ...
    public void deleteNews(String userId, String newsId){
        if (canDeleteNews(userId, newsId)) {
    ...
    private boolean canDeleteNews(String userId, String newsId){
        if (newsId != null && userId != null) {
            MyUser user =manager.find(MyUser.class, userId);
            if (user != null) {
                MyNews myNews = manager.find(MyNews.class, newsId);
                if (myNews != null && user.getOrgs().contains(myNews.getOrg())) {
                    return true;
                }
            }
        }
